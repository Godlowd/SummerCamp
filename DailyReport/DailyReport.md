...
## day10
因为沟通上的失误导致这几天的日报咕咕咕了.....狗头🐶保命
这两天主要完成了用户端生成和刷新二维码的功能，主要的难点是二维码的生成和自动刷新
### 生成二维码
在了解到需要生成二维码之后在网上大概了解了一下二维码的生成原理，最开始是想找网上现有的API当一个调包侠。但是网上找到的并不能很好的符合我的要求，比如生成三色码，最后还是决定自己手撸一个二维码生成器，同时也可以提高弱网条件下的使用效率。
主要是用的是原声框架，利用CIFilter进行相关操作。最开始和网上别的API生成的二维码一样都是黑白两色，然后又参考别的地方的方法，再加了一层色彩滤镜，从而生成指定颜色的二维码。不过虽然很多地方都说生成的二维码会有质量问题放大之后看不清楚，但是就实际操作来看即使放大9倍显示效果依然很好，不需要和其他的方法一样再加一层提升画质的函数。
### 同步更新
不知道为什么不论使用GCD的计时器还是NSTimer的计时器都跑不了emmmm最后还是从别的地方抄来的代码用NSTimer跑通了。等写完了用GCD的试试
### 目前遇到的最大问题
目前用户端基本算是完成了，加密算法考虑使用RSA,但是门禁端的扫描二维码的工作量似乎比我想的还要多。希望两天之内能搞完吧，搞完去搞动画了


## day11
今天的主要精力都放在了门禁端的功能实现上，因为电脑硬件的原因，手机无法和电脑保持稳定的链接，所以调用摄像头部分的功能可能无法实现，今天完成了门禁端从相册读取图片和二维码的功能，遇到的主要问题是无法正常读取二维码中内容
### 无法正常读取二维码
最开始为了所谓的“视觉效果”，给展示二维码的``ImageView``加上了边框，但是后面的二维码死活读不出来，但是用手机微信的扫一扫却能读出来里面的信息。花了几乎一个下午的时间研究到底是怎么回事，最后晚上突发奇想觉得可能是边框的原因影响了二维码的正常的读取，遂取消边框，最后成功读取二维码
### RSA解密
最开始打算采用的加解密方式是RSA，然后在用户端的RSA加密算是成功了，但是在门禁端的解密一直有问题，无法正常的解密，这个问题目前还没有解决，希望明天可以解决。

## day12
今天完成了加解密部分和门禁部分读取完用户提供的二维码之后的动画功能。
### 加解密
加解密主要采用的是RSA加密技术，公钥加密私钥解密，私钥放在门禁处，门禁从二维码读取信息之后利用私钥读出信息并确认是否可以通行。目前的策略是绿色码可以通行，尚未对二维码的有效区间进行判断，明天考虑加入这个功能。使用的加解密的类是网上封装好的轮子，但是在加入公钥和私钥的时候总是遇到了问题，最后勉强解决了。
### 动画
动画主要是用的贝塞尔路径和``CABasicAnimation``。完成了一个简单的画勾和画叉的动画，目前还十分简陋，对于用户端生成二维码时要有的动画目前还没有想法。