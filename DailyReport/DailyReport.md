...
## day13
今天主要完成了昨天版本中扫描完二维码之后有效区间显示错误的问题，以及给更新二维码的时候加了一个动画。
### 有效区间显示问题
在显示的二维码下面我加入了三个按钮，分别用来生成三种不同颜色的二维码，更新二维码的同时需要将计时器的时间重置到最开始的时候。计时器部分使用的是``NSTiemr``。这就要求我们在单击按钮的时候需要无论计时器离更新还剩多少秒，都从头算起。也就是类似于一个重启的功能。最开始在网上找了很久也没有找到重启的方法。最后终于发现一个``setFireDate``复合要求，但是在结果界面的有效区间部分显示的时间区间是更新时间的两倍，断点调试后发现，``setFireDate``会自动执行一次``SEL``.了解了这个特性之后bug就迎刃而解了
### 翻转动画
翻转动画最初的设想是最开始是原来的二维码旋转90度，然后在中间将二维码换成新的二维码。采用的是``CAKeyframeAnimation``，而不是视图转换。如果动画分两次就会在``UIImageView``的``img``更换的时候出现一次奇怪的卡顿。最后还是做成一次，然后让更换二维码在动画开始的一半的时候更换。